...
async def get_google_search_response(query: str) -> str:
    ...
    try:
        ...
        if 'items' in search_results:
            snippets = [re.sub(r'http[s]?://\S+', '', item.get('snippet', '')) for item in search_results['items']]
...
for command in GIF_IMAGE_PATHS.keys():
    ...
    bot_app.add_handler(CommandHandler(command, send_media))
...
bot_app.add_handler(CommandHandler('iespcommands', commands_command))

bot_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
bot_app.add_error_handler(error)

print('Starting bot...')
bot_app.run_polling(poll_interval=1)
